				...
				<el-row>
					...
					<el-col :span="24">
						<el-form-item label="入住时间" prop="ruzhushijian">
							<el-date-picker
								class="list_date"
								v-model="form.ruzhushijian"
								format="YYYY-MM-DD HH:mm:ss"
								value-format="YYYY-MM-DD HH:mm:ss"
								type="datetime"
								:readonly="!isAdd||disabledForm.ruzhushijian?true:false"
								placeholder="请选择入住时间" />
						</el-form-item>
					</el-col>
					<el-col :span="24">
						<el-form-item label="预计离开时间" prop="likaishijian">
							<el-date-picker
								class="list_date"
					...
					<el-col :span="24">
						<el-form-item prop="zhengjian"
									  label="证件"
						>
							<uploads
								:disabled="!isAdd||disabledForm.zhengjian?true:false"
								action="file/upload"

								tip="请上传证件"
								:limit="3"
								style="width: 100%;text-align: left;"
								:fileUrls="form.zhengjian?form.zhengjian:''" 
								@change="zhengjianUploadSuccess">
							</uploads>
						</el-form-item>
					</el-col>
					<el-col :span="24">
...
<script setup>
	...
	//证件上传回调
	const zhengjianUploadSuccess=(e)=>{
		form.value.zhengjian = e
	}
	//methods
	...
	const getInfo = ()=>{
		context?.$http({
			...
			res.data.data.gerenjianjie = res.data.data.gerenjianjie?(res.data.data.gerenjianjie.replace(reg,'../../../cl75385428/file')):'';
	...
	const init=(formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null)=>{
		...
		if(formType == 'add'){
			...
			for(let x in row){
				...
				if(x=='zhengjian'){
					form.value.zhengjian = row[x];
					disabledForm.value.zhengjian = true;
					continue;
				}
				if(x=='gerenjianjie'){
	...
	const save= async ()=>{
		...
		if(form.value.zhengjian!=null) {
			form.value.zhengjian = form.value.zhengjian.replace(new RegExp(context?.$config.url,"g"),"");
		}
