	...
	<div class="detail-page" :style='{}'>
		...
		<div class="detail_view">
			...
			<div class="info_view">
				...
				<div class="btn_view">
					<el-button class="cross_btn" v-if="btnFrontAuth(tableName,'投递')" @click="jianlitoudionAcross('投递','jianlitoudi','','','[1]','已投递')" type="warning">投递</el-button>
					<el-button v-if="centerType&&(detail.ispay=='未支付'||!detail.ispay)&&btnFrontAuth('zhaopinhuodong','支付')" class="approval_btn" @click="payClick">支付</el-button>
					<el-button class="edit_btn" v-if="centerType&&btnAuth('zhaopinhuodong','修改')" type="primary" @click="editClick">修改</el-button>
					<el-button class="del_btn" v-if="centerType&&btnAuth('zhaopinhuodong','删除')" type="danger" @click="delClick">删除</el-button>
				</div>
			</div>
		</div>
		<div class="detail-box">
			<div class="title"><span>岗位详情</span></div>
			<div class="content" v-html="detail.gangweixiangqing"></div>
		</div>
		<div class="detail-box">
			<div class="title"><span>活动详情</span></div>
			<div class="content" v-html="detail.huodongxiangqing"></div>
		</div>
	</div>
</template>
<script setup>
	...
	const jianlitoudionAcross = async (btnType,table,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1) => {
		if(!context?.$toolUtil.storageGet('frontToken')){
